...
def format_columns(data, task_name, to_text=False, zero_shot_ctk=False, insert_prefix=False):
    match task_name:
        case "xnli":
    ...
    if to_text:
        column_names = list(filter(lambda x: x != "label" and x != "guids", data["train"].column_names))
...
def format_ctk(data, zero_shot = False):
    ...
    data = data.align_labels_with_mapping({"SUPPORTS" : 0, "NOT ENOUGH INFO": 1, "REFUTES" : 2}, "label") # xnli style
    ...
    if zero_shot:
        ...
        for split in data:
            data[split].features["label"].names = ["entailment", "neutral", "contradiction"]
    
    return data

def format_nc(data, to_text = False):
    if not to_text:
        data = data.map(lambda x: format_concat("news_title", "news_body", "news_text", x))
...
def format_qadsm(data, to_text = False):
    if not to_text:
        data = data.map(lambda x: format_concat("ad_title", "ad_description", "ad_text", x))
...
def format_wpr(data, to_text=False):
    if not to_text:
        data = data.map(lambda x: format_concat("web_page_title", "web_page_snippet", "web_page_text", x))
