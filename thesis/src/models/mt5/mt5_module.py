...
class MT5(pl.LightningModule):
    ...
    def forward(self, input_ids = None, attention_mask=None, labels=None, inputs_embeds = None, *args, **kwargs):
        return self.model(input_ids = input_ids, inputs_embeds=inputs_embeds, attention_mask=attention_mask, labels=labels)
    ...
    def r3f(self, batch):
        ...
        out = self(attention_mask = batch["attention_mask"],
                   labels=batch["labels"],
    ...
    def generate_predictions(self, input_ids, task_name):
        ...
        predictions = torch.tensor([-1.0 if word not in self.label2id[task_name] else self.label2id[task_name][word] for word in predictions])
        return predictions
