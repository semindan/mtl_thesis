...
class XLMRModule(pl.LightningModule):
    def __init__(self, config, path="xlm-roberta-base"):
        ...
        self.model = XLMRobertaMultiTask.from_pretrained(path, config=config, tasks=self.tasks)
        eps = 1e-5
    ...
    def forward(
        self, input_ids = None, attention_mask = None, labels = None, inputs_embeds=None, task_name = None, *args, **kwargs
    ):
        return self.model(
            input_ids=input_ids,
    ...
    def r3f(self, batch):
        ...
        out = self(attention_mask = batch["attention_mask"], inputs_embeds = inputs_embeds, task_name = batch["task_name"])
        logits = out.logits
    ...
    def get_noised_logits(self, batch, inputs_embeds):
        ...
        noised_out = self(attention_mask = batch["attention_mask"],
